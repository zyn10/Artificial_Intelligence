#18F-0326 Abdul Salam Wasti BFS updation to A* Search
...
class Graph:
    ...
    def AStarSearch(self, s, e):
        print("This is our starting node", s)
        print("This is our ending node",e)
        visited=[]
        queue = []
        path=[]
        parentChild = []            #need parent child track to get path value
        c = s
        parentChild.append(s)
        parentChild.append(c)
        queue.append(parentChild)
        visited.append(s)
        while queue:
            nodetoselect=g.findHeuristicAndPath(queue,huristic,actualvalues)
            c=nodetoselect
            path.append(c)
            print("Node to Select",c)
            parentChild.clear()
            parentChild.append(s)
            parentChild.append(c)
            indexofselectedvalue = queue.index(parentChild)
            queue.pop(indexofselectedvalue)
            if c==e:
                break
            s = c
    ...
    def findHeuristicAndPath(self,pqueu,dich, actualPath):
        #function to compute path value + heuristic value
        ...
        for i in pqueu:
            if i[1] in dich.keys():
                keyslist.append(i[1])
                pathCost = 0
                if actualPath.get( str(i[0]+i[1]) ):   #getting pathvalue by selected node and parent node
                    pathCost = actualPath[str(i[0]+i[1])]
                valueslist.append(dich.get(i[1]) + pathCost)
