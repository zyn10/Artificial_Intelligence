            ...
            while stack:  # while stack is not empty
                ...
                for cityy in self.graph[s]:
                    if cityy not in visited:  # if city is not in visited
                        ...
                        if city_name == cityy:  # if city_name is s
                            return visited  # return visited
                        if len(visited) == depth_limit:
                            return visited
            return visited
    def printDFS_Limited(self,destination):
        path = g.depth_limited_search('Dunwich', destination, 6)  # return dfs path
        nxGraph = nx.Graph()  # creating graph
        for key in g.graph.keys():  # adding edges to graph
            for value in g.graph[key]:
                nxGraph.add_edge(key, value)
        nx.draw(nxGraph, with_labels=True, node_color='green', node_size=2000, font_size=10)  # drawing the graph
        ...
        if len(path) > 0:  # if the vertex is found
            ...
            nx.draw(visitedGraph, with_labels=True, node_color='green', node_size=2000, font_size=10)
            plt.show()
        else:
            print("No path found")
...
g.printDFS()
path = g.depth_limited_search(src, dest)
